...
function changePage(askedPage){
  ...
  new ProductList();
=======
 * @param   {string}  askedPage  le contenu de la barre d'adresse après le point d'interrogation ou les informations de page
 *
 * @return  {void}         appelle la page à afficher
 */
function changePage(askedPage) {
   ...
   if (askedPage.slice(0, 7) === 'product') return new ProductPage(askedPage);
   new ProductList();
   if (askedPage.slice(0, 4) === 'cart') return new Cart(askedPage);
>>>>>>> Stashed changes
}
